...
%\vspace{0.4cm} 
%\Huge{
%RosPiBot\\Restauraci\'on y mejora de una plataforma rob\'otica incorporando un SBC %y ROS}
%\vspace{0.4cm} \\
% Memoria}
...
\mbox{Directores:} & \mbox{Cecilio Angulo Bahon y Manel Velasco Garcia} \\
\mbox{Convocatoria:} & \mbox{Junio 2014}
\end{array}$}\\ 
\vspace{1.5cm} 
...
\addcontentsline{toc}{section}{Resumen}
WifiBot es una plataforma robótica de cuatro ruedas orientada al sector educativo y de desarrollo. Se caracteriza por su capacidad de adaptaci'on a múltiples terrenos.  Este proyecto consiste en el aprovechamiento de la plataforma comercial que corre el riesgo de quedarse obsoleta. Para ello se hace uso de una Raspberry Pi, un ordenador de placa reducida del formato de una tarjeta de cr'edito con un procesador ARM11 utilizado en m'oviles como el IPhone 3G y que ha permitido realizar otros proyectos de este tipo, como unidad de procesamiento de la nueva plataforma y que est'a basada en Linux. \\

%En el apartado de software, adem'as de incorporar herramientas que faciliten el trabajo a futuros usuarios se ha configurado para poder trabajar con ROS, una infrastructura digital para el desarrollo de software de robots creada por Willow Garage y extensamente utilizada en este sector.\\
...
\item[ROS] Robot Operative System
\item[SLAM] Simultaneous Localization And Mapping
\item[RoHS] Restriction of Hazardous Substances Directive
\end{description}
...
A mediados de 2007, el departamento de de Ingeniería de Sistemas, Automática e Informática Industrial (ESAII) adquiri'o un par de plataformas rob'oticas conocidas como WifiBot4G. Este producto ofrec'ia un robot capaz de navegar por terrenos abruptos gracias a su tracci'on de cuatro ruedas y deserrollar nuevos proyectos.\\

Este trabajo surge de la necesidad de recuperar una plataforma r'obotica que ha dejado ser utilizada por estudiantes a causa de los problema derivados de su unidad de procesamiento y de la falta de informaci'on relacionada con el robot. El hecho de recurrir a software desactualizado y que ya no dispone de soporte por parte de su fabricante, ha relegado este robot a formar parte de unos activos que ya no volver'an a ser utilizados. Mec'anicamente los motores siguen aportando las mismas caracter'isticas que al principio y los componentes m'as b'asicos siguen utiliz'andose en la actualidad.\\

%En tiempos de escasez pero de avances tecnol'ogicos, uno empieza a descubrir el potencial que esconde este tipo de plataformas. 
%Mec'anicamente los motores siguen aportando las mismas caracter'isticas que al principio y los componentes m'as b'asicos siguen utiliz'andose en la actualidad. Su 'unico punto d'ebil es un procesador desfasado (pero adelantado en su 'epoca).\\

%Y es en el momento de buscar el proyecto, uno se pregunta tipo de trabajo quiere realizar. En este caso, el objetivo era crear un proyecto que tuviese continuidad después de su realizaci'on. As'i pues, se ha considerado que la restauraci'on de un plataforma rob'otica obsoleta puede convertirse en ese proyecto deseado. Lo solo se recuper'a una herramienta educacional, sino que al mejorarla, permitir'a a futuros estudiantes llevar a cabo nuevo trabajos.\\

Durante la realizaci'on del proyecto se definieron las dos vertientes del WifiBot donde concentrar esfuerzos, el hardware y el software. Se analiz'o la viabilidad de mantener el hardware ya existente, pero se concluye que es mejor redise'narlo. Por lo que afecta al software, se decide dar un fuerte empuje al futuro. Con este objetivo se ha trabajado en lenguaje Python por el protagonismo est'a adquiriendo d'ia a d'ia y se ha instalado un infraestructura digital para el desarrollo de robots conocida como ROS.

%\newpage
%\subsection{Objetivos}
%El objetivo principal de este proyecto es recuperar la plataforma WifiBot. Para ello no solo se restaurar'a, sino que se mejorar'a para ofrecer una interacci'on m'as c'omoda con el usuario. Tal y como se ha comentado en la introducci'on, el principal problema se centra en su procesador; Por ello el primer paso ser'a buscar un subtituto que pueda cumplir la labor. Adem'as para evitar que se repita la situaci'on. se va a elegir un componente que goce de una amplia repercusi'on, que disponga de una gran comunidad a su espalda y que no requiera de extensa documentaci'on para poder trabajar con él.\\

%Una vez mejorado el cerebro del robot, se atacar'an otros punto d'ebiles localizados durante el largo periodo de observaci'on que se experiment'o. Una respuesta luminosa o ac'ustica permitir'ia al usuario conocer algunos estados de la plataforma como por ejemplo: Si los dispositivos est'an operativos, si hay comunicaci'on entre ellos e incluso si dispone de carga en su bater'ia.\\
%
%Otro campo a mejorar ser'ia el apartado de sensores para percibir informaci'on de su entorno. Ya que se trata de un robot dise'nado en su or'igen para moverse por terreno abrupto, ser'ia recomendable reforzar este sector y disponer se sensores acorde a dise'no.\\
%
%Con una plataforma actualizada, el siguiente tema que deber'iamos centrar nuestros esfuerzos, ser'ia en encontrar de que manera podemos hacer apetecible el robot para que atraiga la mirada y transmita ganas de experimentar. Ofrecer un conjunto de herramientas que permita un acceso a todo el robot pero que a la vez evite problemas indeseados.

\newpage
\section{Objetivos}
\label{obje}
Puesto que el objetivo primordial es mejorar la unidad de procesamiento, se ha elegido que entre todas las opciones de introducir una Raspberry Pi. Esta placa cuenta con un gran nombre de perif'ericos que se adaptan perfectamente a las necesidades del robot. Como caracter'isticas t'ecnicas m'as destacables, puede ejecutar un sistema operativo basado en Linux, cuenta con conectores de red y USB adem'as de pines para m'ultiples funciones; Otra cualidad remarcable es que su disco duro es extra'ible, de esta manera se puede manipular el sistema sin la necesidad de estar obligatoriamente siempre conectado al robot.\\

En el caso de los sensores externos, un conjunto de sensores de distancia, una c'amara y un m'odulo IMU ser'an una buena opci'on a la hora de actualizar el robot. De esta manera se evita el problema de la interferencia de la luz solar en los sensores infrarrojos, adem'as de ofrecer otras informaciones como la orientaci'on del robot e incluso poder aplicar procesamiento de imagen.\\

Adem'as de sensores, se hace necesaria una remodelaci'on tanto del exterior como del interior del robot. Ofreciendo un sistema m'as c'omodo a la hora de conectarse, tanto por cable como de modo inal'ambrico. A'un as'i, se mantendr'a la estética de un robot formado por dos hemisferios los cuales pueden rotar respecto un eje transversal.\\

Para incitar al futuro usuario a trabajar con la nueva plataforma, se ofrecer'a un conjunto de funciones en lenguaje Python con las que obtener informaci'on del entorno y hacer mover el robot se convierta en una labor verdaderamente simple. Para ello se crear'an dos grupos de c'odigos. Los primeros se encargar'an de la comunicaci'on con los diferentes dispositivos y almacenar'an la informaci'on. El segundo grupo tendr'a la funci'on de ofrecer herramientas para que el usuario sea capaz de recuperar la informaci'on guardada.\\

Finalmente, para darle un valor a'nadido al robot, se instalar'a ROS, este conjunto de herramientas est'a orientado para plataformas rob'oticas. El hecho de lanzar un n'ucleo de ROS dentro del robot ofrece un mayor n'umero de posibilidades a futuros trabajos. En este proyecto 'unicamente se plantea la opci'on de instalar y configurar ROS. Para futuros proyectos se recomienda la lectura de libros que nos introduzcan en este mundo.\cite{ros}.

\newpage
\section{Alcance}
Debido a que el estudio de la viabilidad de reutilizar el hardware ya existente demostr'o que ser'ia recomendable redise'narlo, se procedi'o a estudiar las alternativas. A causa de deshacernos de todo el hardware, incluida la etapa de potencia de motores, gestión de baterías y lectura de sensores, fue necesario la obtención de dispositivos que pudiesen ofrecer las mismas características y que la Raspberry Pi no disponía. Aunque en el mercado existía gran variedad de productos que desempeñaban algunas de las funciones necesarias, no se localizó ninguna que ofreciera los que se buscaba. Por este motivo se decidió desarrollar un hardware que satisficiera todas nuestras necesidades. \\

Así pues, se han creado tres dispositivos distintos: La placa SecurePi, que permite alimentar la lógica con un rango de 6 a 35V y además protege todas las salidas del GPIO de la Raspberry Pi (pasándolas de 3.3V a 5V). La placa de expansi'on SensorPi, diseñada para encajar sobre la SecurePi, que ofrece la contabilización de dos codificares de cuadratura, ocho canales de conversión analógico a digital y ocho pines que pueden utilizarse tanto para inputs o output. Y finalmente la placa MotorPi, que permite el control de hasta cuatro motores, ofreciendo lectura de intensidad de cada uno, además incorpora un volt'imetro con representación de 10 luces que indican el estado de carga de la batería.\\

Puesto que el interior se iba a manipular, se decidió aprovechar para mejorar otros aspectos. La primer punto fue ofrecer un sistema de encendido más cómodo, en vez de un interruptor interior se substituyó por un antivandálico situado en la parte trasera y de fácil acceso. Las baterías pasaron de encontrarse en el exterior a formar parte del interior, se cambiaron por unas de polímero de litio con una electrónica de gestión de carga propia. De esta manera no haría falta comprar cargadores específicos, simplemente una fuente de alimentación acorde de 9V. Para poder acceder remotamente al robot se optó por la incorporación de un router WiFi comercial, para mejorar la modularidad del nuevo robot. por lo que afecta a los nuevos sensores, se instalaron los definidos anteriormente en en el apratado \ref{obje}. A causa de las nuevas modificaciones internas, se rediseñó el exterior del robot, aumentando la altura del chasis pero manteniendo la distancia entre ejes. Puesto que el objetivo era mantener la estética del robot, se conservó su característica principal, un eje transversal que permite pivotar las dos partes del robot.\\

Finalmente se ha desarrollado un módulo de Python llamado rospibot que permite al usuario comandar y acceder a toda la información necesaria, además de instalar y configurar ROS. Este proyecto no otorga ninguna funcionalidad al robot, simplemente se ha centrado en el objetivo de ofrecer una plataforma lo más flexible posible para cualquier futuro proyecto.

\newpage
\section{Descripci'on Wifibot}
\textbf{Wifibot} es una plataforma rob'otica, que se empezó a desarrollar en el 2004 por la empresa francesa \textbf{Nexter Robotics} (anteriormente conocida como GIAT Industries o el 
\textit{Groupement des Industries de l$'$Armée de Terre}, Grupo de Industrias Militares del Ejército de Francia), dise'nada para poder navegar en m'ultiples escenarios gracias a su dise'no. Su sistema de tracci'on a las cuatro ruedas, dise'no reducido ,bajo peso y baterías intercambiables, le otorgan una gran flexibilidad. 

\subsection{Caracter'isticas}
Este proyecto parte del modelo \textbf{WifiBot 4G} (figura \ref{fig:Wifi Bot 4G}), producido durante el periodo 2005--2007. Se trata de una versión mejorada del WifiBot SC que cuenta con un mejor procesador y módulo de red.
Seg'un su gu'ia de usuario \cite{guia}  est'a formador por:
...
\item CPU:
	...
	\item 400MHz
	\item Memoria RAM de 64MB
	\item Memoria Flash de 32MB\\
	\end{itemize}
\item Interf'icies:
	\begin{itemize}
...
\item Red:
	...
	\item Modos AP, Bridge y Router
	\item Antena de 5dBi
	\item Switch de 5 puertos\\
	\end{itemize}
\item Dimensiones:
	\begin{itemize}
	\item Longitud 28cm
...
\item Sensores:
	\begin{itemize}
	%\item 1x C'amara IP
	\item 2x Sensores IR de dist'ancia
...
La estructura de la base  que se muestra en la  figura \ref{fig:wifibot-open} est'a formada por dos secciones sim'etricas que llamaremos hemisferios izquierdo y derecho. Estas dos partes est'an unidas por una barra roscada que atraviesa transversalmente todo el robot, ofreciendo un eje de rotaci'on entre los dos elementos, cualidad que le otorga una mayor adaptaci'on a superf'icies irregulares. En la figura \ref{fig:wifibot-simple} se puede apreciar la barra roscada que atraviesa todo el robot en el eje transversal, además de los paneles que forman el chasis del robot y la distribución de los motores  con sus acopladores.
%include image SolidWorks
%como vemos en figura \ref{fig:Wifi Bot 4G}

\begin{figure}[ht]
\centering
\includegraphics[scale=0.1]{images/wifibot-open.png} 
\caption{WifiBot parcialmente desmontado}
\label{fig:wifibot-open}
\end{figure}

\subsection{Distribución del hardware}
El hardware interno de divide en cuatro elementos, su distribución se peude observar en la figura \ref{fig:wifibot-simple}: En el hemisferio izquierdo encontramos el ordenador de abordo con la tarjeta de expansión WiFi y la placa encargada de la gestión de baterías y alimentación de los demás componentes. Por el hemisferio, el switch y la placa de lectura de sensores y control de motores.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.07]{images/wifibot-simple.png} 
\caption{Distribución de los elmentos internos del WifiBot}
\label{fig:wifibot-simple}
\end{figure}

\subsection{Grupo Motriz}
Formado por cuatro motores, concretamente el modelo \textbf{HN-GH7.2-2414T-50:1} del fabricante \textbf{Hsiang Neng}. De los datos extraídos de su ficha técnica \cite{motor}, nos indican que este modelo est'a compuesto por un motor de continua de voltaje nominal de 7.2V y una reductora con relaci'on 50:1. Dispone de un eje de 6mm de di'ametro, una longitud de 81.1mm y un di'ametro exterior de 37mm. Además el fabricante nos incluye una gráfica con las curvas específicas, figura \ref{fig:Gr'afica del Motor}, con los datos de potencia de salida (en Vatios), el consumo, la eficacia y la velocidad de salida respecto el par.
...
En mediciones efectuadas, se determin'o que la alimentaci'on de los motores llegaba hasta los 8.2V con un consumo inferior a los 1.8A por motor (con carga), bas'andonos en curva de consumo, podremos asegurar que el motor se encontraba ofreciendo un par de 4.7 Kg-cm.

\subsection{CPU y red}
A diferencia del modelo SC, el WifiBot4G incorpora la lógica de control y el dispositivo de comunicación inalámbrico en un mismo producto, un m'dulo creado por la empresa \textbf{4G Systems}. En este caso de trata de de un ordenador embebido parecido al estándar PC/104 y que se comunica con la trajeta de expansión WiFi mediante un conector PCI.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.08]{images/cube4G.png} 
\caption{Ordenador embebido con tarjeta de expansión WiFi.}
\label{fig:cube4G}
\end{figure}

\subsection{Sensor infrarrojo}
El robot está equipado con dos sensores infrarrojos GP2Y0A02YK del fabricante \textbf{Sharp Corporation} con un rango efectivo de 20cm hasta un máximo de 150cm. Su funcionamiento se basa en la triangulación y emiten una señal analógica inversamente proporcional a la distancia del obstáculo. La salida del sensor no es lineal y sigue una cierta curva definida en la figura \ref{fig:graph-sharp}. Se pueden encontrar más especificaciones en su ficha técnica \cite{sharp}

\begin{figure}[H]
\centering
\includegraphics[scale=0.390]{images/graph-sharp.png} 
\caption{Sensor y gráfica de la salida analógica respecto distancia.}
\label{fig:graph-sharp}
\end{figure}

\subsection{Codificador 'optico de cuadratura}
De los 4 motores que conforman la plataforma, los dos delanteros disponen de un codificador 'optico, concretamente el modelo \textbf{E4P-120-079-HT} del fabricante \textbf{US DIGITAL}. Este tipo de sensor permite conocer tanto la velocidad como la posici'on de un eje.
 
\begin{figure}[H]
\centering
\includegraphics[scale=0.40]{images/encoderQEI.png} 
...
\begin{tabular}{|c|c|c|}
	\hline
	\textbf{Voltaje [V]} & \textbf{Consumo [A]} & \textbf{Per'iodo de Cuadratura [$\mu$s]} \\ \hline
	0.5 & 0.07 & 2200 \\ \hline
	1.0 & 0.08 & 1000 \\ \hline
	1.5 & 0.09 & 320 \\ \hline
	2.0 & 0.10 & 220 \\ \hline
...
\subsection{GitHub }
Con el objetivo de ofrecer toda la informaci'on necesaria a futuros usuarios se ha creado el repositorio \url{https://github.com/Guasch5000/RosPiBot}, que permitir'a acceder a todos los archivos relacionados con este proyecto, desde c'odigos de programaci'on, hasta los dise'nos de los componentes a utilizar. De este modo se consigue que la informaci'on perdure, sea accesible y permite ponerse en contacto con su desarrollador.  

\subsection{Librer'ias}
...
Las bater'ias ofrecer'an  una autonom'ia suficiente para moverse en un entorno exterior, se estipula un m'inimo de 2 horas en movimiento como m'inimo aceptable. Adem'as de la duraci'on, su recarga deber'a de ser lo más cómodo posible para el usuario, evitando que tenga que manipular las celdas de energ'ia. Se recomienda elementos de seguridad ante problemas como sobrecargas o cortocircuitos y de gesti'on de energ'ia para aumentar la vida 'util.\\

Otro punto a tener en cuenta es la disponibilidad en encontrar repuestos de la bater'ia, por ello se usar'an productos comerciales f'aciles de adquirir. Finalmente, se monitorizar'a el nivel de carga a partir de elementos luminosos.

\subsection{L'ogica}
Para el control de la plataforma el proyecto se basa en uso de la Raspberry Pi. Dicho ordenador ser'a el responsable de efectuar todo los c'alculos y 'ordenes. A'un as'i, a causa de sus limitaciones se precisa de una l'ogica complementaria capaz de gestionar y contabilizar las lecturas anal'ogicas de los sensores, el control de motores, la comunicaci'on externa y la distribuci'on de energ'ia.
...
Debido al 'impetu de los desarrolladores de la Raspberry Pi por ofrecer una herramienta para las escuelas, esta placa posee un conjunto de librer'ias para ser programada en Python. Adem'as, cada vez hay m'as desarrolladores independientes que ofrecen m'odulos muy 'utiles y nos ayudan a ahorrar tiempo, permitiéndonos crear m'as r'apidamente nuestros propios proyectos.


\subsubsection{GPIO}
Uno de los grandes alicientes de esta placa comparada con otros ordenadores embebidos es el hecho de disponer de GPIOs, este elemento hace referencia al conjunto de pines que dispone la placa y pueden ser configurados como entradas o salidas digitales. Estos pines est'an conectados directamente a la CPU y sus niveles de operaci'on se encuentra en los 0 o 3,3V. Por contra no pueden ofrecer m'as de 16mA por pin, y un m'aximo de unos 200mA en su totalidad.
fritzing-raspberry

\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{images/fritzing-raspberry.png} 
\caption{Ubicación GPIO}
\label{fig:fritzing-raspberry}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{images/gpio.png} 
\caption{Nomenclatura del GPIO}
\label{fig:gpio}
\end{figure}

Aunque todos los pines definidos como GPIO pueden usarse como entradas o salidas, algunos de estos disponen de otras funciones. Este es caso de los protocolos de comunicación por serie, es decir que los bits se envían siguiendo un orden por un mismo canal (cable). De buses disponemos síncronos y asíncronos, en el primer grupo encontramos el bus I$^{2}$C, que requiere de un único cable de datos(comunicación bidirecional), y el SPI, que utiliza un canal para cada sentido. Estos dos buses permiten la comunicación con varios dispositivos conectados al bus, pero requieren de un ''master'' que gestione el bus. En el segundo grupo encontramos el bus UART, este bus solo permite interaccionar dos dispositivos que previamente han acordado que protocolo seguirán. \\


En el caso del protocolo de comunicaci'on I$^{2}$C, el ping GPIO2 se convierte en \textbf{SDA} y el GPIO3 en \textbf{SCL}. Para el bus SPI, el GPIO10 pasa a ser \textbf{MOSI}, el GPIO9 a \textbf{MISO}, el GPIO11 a \textbf{SCLK} y los pines de selecci'on de esclavo GPIO8 para el \textbf{CE0} y GPIO7 para el \textbf{CE1}. Finalmente la comunicaci'on UART, \textbf{TXD} por el GPIO14 y \textbf{RXD} por el GPIO15.

\subsubsection{Sistemas Operativos}
Debido a la gran revoluci'on que han supuesto este tipo de ordenadores, se han creado a su alrededor m'as de 20 sistemas operativos para la Raspberry Pi, los m'as utilizados basados en Linux. Dentro de la comunidad que hay detr'as de este pequeño ordenador, el sistema m'as utilizado con finalidades de disponer de un ordenador de bolsillo es Raspbian. Este sistema es una versi'on de Debian Wheezy para ARMv6 con soporte para coma flotante por hardware.

%\begin{figure}[H]
%\centering
%\includegraphics[scale=0.2]{images/raspbian.png} 
%\caption{Icono de S.O. Raspbian}
%\label{fig:raspbian}
%\end{figure}

%\subsection{Motorizaci'on}
%Debido al buen estado de los motores y del hecho que un par incorpora codificadores de cuadratura, se ha optado por mantener estos motores. Con este gesto evitamos la b'usqueda de su substituto y nos aseguramos que se mantenga la misma funcionalidad que al inicio. Al conservar este dispositivos se considera que el consumo (por lo que afecta a la parte de motorizaci'on) permanecer'a semejante, por tanto la nueva etapa de control de motores dispondr'a de caracter'i­sticas similares. Siendo el voltaje nominal del grupo motriz de unos 7.2 se requiere de una alimentaci'on acorde a este voltaje; En el caso de utilizar bater'ias de voltaje superior, ser'a necesario una etapa DC-DC para adecuar los niveles.
...
Las caracter'isticas de este sensor han sido el motivo de su elecci'on, pueden encontrarse en su ficha t'ecnica \cite{sonar}: Su gran rango de medida (de 1cm a 550cm), una resoluci'on de 1mm, comunicaci'on via I${^2}$C (con 20 direcciones possibles), capaz de operar de 3.0 a 5.5V y permite hasta unas 500 lecturas por segundo. Además, partiendo del diagrama de radicación (figura \ref{fig:KS103_diagram}) podemos considerar un cono de unos 40 grados de obertura. Otro factor que se ha tenido en cuenta es el hecho que la luz de sol en determinadas circunstancias puede afectar a la lectura real de los sensores infrarrojos, en cambio los sensores de ultrasonidos no quedan afectados.\\
...
\subsection{IMU}
Además de sensores de distancia por ultrasonidos, los sensores que permiten conocer la orientación del robot son cada vez más solicitados en las nuevas plataformas. Por este motivo se ha decidido añadir el dispositivo GY-80, formado por tres acelerómetros, tres giróscopos, tres magnetómetros y un sensor de presión. De esta manera se obtiene información de de cada uno de los ejes y permite la elaboración de sistemas de navegación inerciales. Otro punto fuerte de este dispositivo es que permite tanto la comunicación por I${^2}$C como por SPI, ofreciendo una gran flexibilidad.

\begin{figure}[H]
\centering
\includegraphics[scale=0.20]{images/gy80.png}
\caption{IMU de 10 grados GY-80}
\label{fig:gy80}
\end{figure}
...
Finalmente las la bater'ia elegidas ha sido un conjunto formado por varias celdas de 3.7V con un modulo PCM instalado hasta obtener unos 11.000mAh. Su consumo de carga llega a los 4A y a unos 12A en descarga.

\subsection{Punto de acceso WiFi} \label{sec:router}
...
Seg'un su gu'ia de usuario \cite{router}, entre todas sus configuraciones (AP Mode, Router Mode, Client Mode,
...
\subsection{Mando Inalámbrico}
La experiencia recolectada en otros proyectos de robótica ha demostrado que ofrecer un sistema de teleoperación simple sin la necesidad de un ordenador permanentemente conectado puede ahorrar muchas horas de trabajo. Por este motivo se ha elegido incorporar un mando inalámbrico F-710 de Logitech, un mando que goza de una gran aceptación dentro de los proyectos realizados conjuntamente con una Raspberry Pi.

\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{images/f710.png} 
\caption{Mando inalámbrico F-710 de Logitech}
\label{fig:f710}
\end{figure}

Gracias a que este mando proporciona un diminuto receptor por USB, se utilizará uno de los puerto ethernet que dispone la Raspberry Pi. De esta manera la comunicación se hace directamente con la unidad de control y  permite conducir el robot se manera simple. 


\subsection{Conmutador de red}
...
\subsection{Descodificador de cuadratura} \label{sec:LS7366}
...
Para solventar el problema, se hace uso del circuito integrado LS7366 \cite{counter}, un contador de cuadratura de hasta 32 bits con interf'icie SPI. Este componente incrementa o decrementa un contador interno dependiendo de la secuencia que proviene del codificador de cuadratura. Adem'as en el momento que se quiera acceder al registro de dicho contador, el circuito procede a copiar el estado de contador en un segundo registro, permitiendo que la lectura no interfiera con el c'omputo.\\
...
\subsection{Adaptador de nivel de tensi'on}
...
Debido a que algunos pines requieren bidirecionalidad (por ejemplo I${^2}$C) se ha decantado por el uso del circuito recomendado por la empresa \textbf{Philips Semiconductors} explicado en su documento \textbf{AN97055} \cite{bidirectional} y que corresponde a la figura \ref{fig:Level_shifter_circuit}. Finalmente para el circuito se utilizar'an resistencias de 10K$\Omega$ para la funci'on de ''pull-up'' y el mosfet de canal N \textbf{BSS138} \cite{bss138}como el de la figura \ref{fig:BSS138}.


\subsection{Alimentaci'on de l'ogica}
...
\subsection{Control de motores} \label{sec:L298N}
...
\section{Integraci'on de los componentes}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.40]{images/sensorpi.png}
\caption{Layout y PCB de la placa SensorPi}
\label{fig:sensorpi}
\end{figure} 
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.40]{images/motorpi.png}
\caption{Layout y PCB de la placa MotorPi}
\label{fig:motorpi}
\end{figure} 
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.34]{images/esquema_power.png}
...
\newpage
\section{Componentes mec'anicos}
...
\subsection{Grupos individuales}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.13]{images/rospibot_finish.png}
...
\section{Configuraci'on Router WiFi}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{images/routerIP.png}
\caption{Configuraci'on IP del router.}
\label{fig:routerIP}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{images/dhcprospibot.png}
\caption{Configuraci'on servidor DCHP.}
\label{fig:dhcprospibot}
\end{figure} 

\newpage
\section{Configuraci'on Raspberry Pi}
...
\subsection{Grabaci'on Raspbian}
...
\subsection{Instalaci'on de librer'ias}
...
\caption{Archivo de configuraci'on de los m'odulos.}
\label{fig:modules}
\end{figure} 
...
Todos los c'odigos que aqu'i se mencionan pueden hallarse en el repositorio \textbf{GitHub} de este proyecto, \url{https://github.com/Guasch5000/RosPiBot}. A continuaci'on se detallan los programas ubicados en la carpeta  \textbf{Publishers}, cual es su funci'on, que par'ametros intervienen y de que manera almacena la informaci'on.
...
%\newpage
\section{ROS}
El nombre de ROS viene de ''Robot Operative System''. ROS puede clasificarse como un Framework de Desarrollo en Robótica que ofrece herramientas y librerías para el desarrollo de sistemas rob'oticos. A pesar de su nombre, ROS no es un sistema operativo propiamente dicho (de hecho funciona sobre Linux).\\

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{images/ros.png} 
\caption{Logotipo de ROS}
\label{fig:ros}
\end{figure}

El objetivo de ROS es ofrecer una solución integral al problema del desarrollo de robots. Para ello requiere de una abstracci'on del hardware, de tal manera que cada plataforma entienda la misma orden, es decir que si ROS requiere que el robot se desplace hacia adelante, pues este lo cumpla independientemente de si se trata de un robot diferencial, b'ipedo o volador. Por su parte, ROS incorpora  desde algoritmos de bajo nivel de control, cinemática, SLAM, etc., hasta algoritmos de alto nivel como planificación o aprendizaje.\\

Lo que hace de ROS una herramienta tan extendida en el mundo de la rob'otica es su mecanismo de comunicaciones distribuido entre nodos. Un nodo es cualquier c'odigo de software del sistema (desde un algoritmo SLAM hasta un driver para el manejo de un motor). El hecho de usar nodos obliga a realizar un nivel de abstracci'on, de esta manera podemos asignar tareas para cada uno de los nodos y conseguir que muchos de estos se puedan reutilizar. 
%Otra caracter'istica importante del uso de nodos es la independencia de donde se encuentre localizado el nodo, ya que ROS no hace distinciones.\\

Estos nodos se comunican entre ellos mediante mecanismos de intercambio de mensajes, entre estos ROS nos ofrece \textbf{Topics}, \textbf{Publishers}, \textbf{Subscribers} y  \textbf{Services} entre otros. Para entender sus diferencias se propone un ejemplo siguiendo la figura \ref{fig:rxgraph}: Imaginemos que tenemos un nodo que llamaremos \textbf{teleop$\_$turtle} y que informa de la velocidad de un robot, debido a que la velocidad puede variar en cualquier momento, se ha programado el nodo para que informe cada 0,1 segundos. En este caso el nodo publica la informaci'on, a este acto se define como ''publicar'' (Publisher). La informaci'on que env'ia se conoce como ''Topic'' (es renombrado como \textbf{/turtle1/command$\_$velocity} ) y su estructura ''Message''. Todo esto se define en archivos de configuraci'on o dentro del propio c'odigo del nodo. \\

\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{images/rxgraph.png} 
\caption{Gr'afico ejemplo de la comunicaci'on entre nodos.}
\label{fig:rxgraph}
\end{figure}

Siguiendo con el ejemplo, si ahora hemos creado un nodo (llamado \textbf{turtlesim)}que se dedica a estudiar la odometr'ia, haremos que se subscriba al t'opico, es decir hemos creado un ''Subscriber''. Para acabar hacemos que cada nodo también publique en el tópico \textbf{/rosout} información complementaria.

\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{images/gazebo.png} 
\caption{Simulaci'on del robot PR2 en Gazebo y publicaci'on en Rviz.}
\label{fig:gazebo}
\end{figure}

Adem'as de los t'opicos, tambi'en existen los servicios, los servicio se diferencian en que un nodo llama a otro para intercambiar informaci'on. esta informaci'on no puede ser compartida entre otros nodos, excepto si estos utilizan el mismo servicio. Es decir, un t'opico puede ser publicado por varios nodos, y varios nodos se pueden subscribirse a él. Pero un ''Service'' solo funciona entre dos nodos.\\


Otro punto fuerte de ROS es la incorporaci'on de simuladores virtuales donde podemos  crear un modelo de nuestro robot y simular su comportamiento en cualquier ambiente, Gazebo y Rviz. Durante la simulaci'on, el sistema es capaz de ofrecernos informaci'on sobre la din'amica de nuestro robot, obteniendo unos resultados muy parecidos a la realidad sin la necesidad que disponer físicamente del robot. 
...
De las versiones existentes en la realizaci'on del proyecto, solo hab'ian dos posibilidades, \textbf{Groovy} o \textbf{Hydro} (la distribuci'on de Indigo a'un se encontraba en desarrollo). De entra las dos versiones disponibles se elegi'o Groovy debido a que ofreci'a soporte directo a Raspbian.\\
...
\newpage
\chapter{Presupuesto}
El coste total de este proyecto asciende hasta los \textbf{11.139,69 euros}. El material utilizado implica unos \textbf{523,69 euros} (un 4,7\% del coste total) y el coste personal aproximado de unos \textbf{10.600,00 euros} (el 95,3\% restante). Su realización se ha efectuado en un plazo de 15 semanas y ha supuesto unas 540 horas de dedicación. Aunque el coste real solo ha sido el gasto en material, hay que ser consciente de cuanto supone el tiempo invertido. A continuaci'on se detallan los costes del proyecto, especificando cada uno de los componentes:
...
\subsection{Coste de Dispositivos}

\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
	\textbf{Concepto} & \textbf{Coste unitario [\euro /u]} & \textbf{Unidades} & \textbf{Total [\euro]}\\
	 \hline
...
\subsection{Coste de la Electr'onica}
\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
	\textbf{Concepto} & \textbf{Coste unitario [\euro /u]} & \textbf{Unidades}& \textbf{Total [\euro]} \\ \hline
...
\subsection{Coste placa SecurePi}
\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
	...
	Resistencia 10Kohms SMD 1206 & 0,005 & 32 & 0,15\\ \hline
...
\subsection{Coste de la Mec'anica}
\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
...
\subsection{Coste placa SensorPi}
\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
	...
	Led verde & 0,05 & 1 & 0,05\\ \hline
	Resistencia 1Mohms 1/4W SMD 1206 & 0,01 & 2 & 0,02\\ \hline
	Resistencia 10Kohms 1/4W SMD 1206 & 0,005 & 12 & 0,06\\ \hline
	Resistencia 6,9Kohms 1/4W SMD 1206 & 0,007 & 4 & 0,03\\ \hline
	Resistencia 2,49Kohms 1/4W SMD 1206 & 0,008 & 2 & 0,02\\ \hline
	Resistencia 1Mohms 1/4W SMD 1206 & 0,01 & 2 & 0,02\\ \hline
	Resistencia 1Mohms 1/4W SMD 1206 & 0,01 & 2 & 0,02\\ \hline
...
\subsection{Coste placa MotorPi}
\begin{table}[H]
\begin{center}
\begin{tabular}{|l|r|r|r|}
	\hline
	...
	Resistencia 10Kohms 1\% DIP & 0,03 & 2 & 0,06\\ \hline
	Resistencia 11Kohms 1\% DIP & 0,03 & 1 & 0,03\\ \hline
	Resistencia 13Kohms 1\% DIP & 0,10 & 1 & 0,10\\ \hline
	Resistencia 1Kohms SMD 1206 & 0,007 & 4 & 0,03\\ \hline
	Resistencia Shunt 0,5Ohms 2W SMD & 1,91 & 4 & 4,74\\ \hline
...
\begin{tabular}{|l|r|r|r|}
	...
	Estudio de viabilidad & 30 & 100 & 3.000\\ \hline
	Dise'no circuito impreso & 25 & 30 & 750\\ \hline
	Dise'no asistido por ordenador & 25 & 70 & 1.750\\ \hline
	Soldadura PCB & 15 & 60 & 900\\ \hline
	Montaje y cableado & 15 & 70 & 1.050\\ \hline
	Programaci'on & 15 & 210 & 3.150\\ \hline
	\textbf{Total} &  & \textbf{540} & \textbf{10.600} \\ \hline
...
\subsection{Descripci'on de las labores}
...
Este proyecto se ha realizando en un periodo de 15 semanas. En el diagrama de la figura \ref{fig:diagramagantt} se aprecia como determinadas labores se han realizado al mismo tiempo. El motivo de dicha situaci'on se debe al continuo desarrollo de los diferentes sectores y del hecho que decisiones en un sector pueden afectar a otros directamente.\\

Para la programaci'on final del robot se esper'o a disponer de la plataforma totalmente ensamblada, con el fin de poder realizar las pruebas directamente sobre el robot. De esta manera se evitaba repetir las comprobaciones ya que su elaboraci'on previa no implicar'ia un ahorro de tiempo.\\
...
\gantttitlelist{0,...,16}{1} \\
\ganttbar{Estudio de viabilidad}{1}{3} \\

\ganttbar{Modelado 3D }{4}{9}\\
\ganttbar{Mecanizado de piezas }{10}{11}\\
\ganttbar{Ensamblado del robot }{12}{13}\\

\ganttbar{Dise'no PCBs }{4}{5}\\
\ganttbar{Producci'on PCBs }{6}{9}\\
\ganttbar{Montaje de las PCBs }{10}{10}\\

\ganttbar{Configuraci'on Raspberry }{4}{4}\\
\ganttbar{Instalaci'on ROS }{5}{5}\\
\ganttbar{Programaci'on Sensores y Motores }{5}{13}\\
\ganttbar{Programaci'on m'odulo Rospibot }{14}{15}\\

\ganttlink{elem0}{elem1}
\ganttlink{elem1}{elem2}
\ganttlink{elem2}{elem3}
\ganttlink[link mid=.9286]{elem3}{elem10}

\ganttlink[link mid=.125]{elem0}{elem4}
\ganttlink{elem4}{elem5}
\ganttlink{elem5}{elem6}
\ganttlink{elem6}{elem3}

\ganttlink[link mid=.0714]{elem0}{elem7}
...
\section{Elementos suprimidos}
Aunque este proyecto trate de la restauraci'on de un plataforma m'obil que estaba en desuso, hay que tener en cuenta que existe la posibilidad que su mejora suponga un empeoramiento del medioambiente. Para ello se ha tenido en cuenta aquello de lo que nos deshacemos y de lo que añadimos.\\

Los paneles originales del WifiBot est'an constituidos de Policarbonato, este tipo de pl'astico se recicla alrededor del 90\% en pa'ises europeos. Su impacto en el ambiente es reducido y se puede encontrar en aplicaciones sanitarias. Aun así, desde hace unos a'nos se ha comprobado que en condiciones de degradaci'on este pl'astico libera mol'eculas de Bisfenol A, que puede causar anormalidades durante el desarrollo de embriones.\\ 

Otro elemento del que nos desharemos ser'a la circuiter'ia original. Debido al uso de esta'no con plomo hasta 2006 por la directiva RoHS, muy probablemente que contenga este metal pesado. Adem'as de plomo, a causa de la complejidad de la circuiter'ia empleada es posible que tambi'en disponga de otro metales nocivos como el zinc, cobalto o n'iquel entre otros. Para minimizar su impacto, los circuitos ser'an entregados en un punto especial de recogida de este tipo de material para su reciclaje.\\

Adem'as de la circuiter'ia, las bater'ias tambi'en s'on otro elemento a tener en cuenta. Aunque la eliminación inadecuada de las baterías de NiMH representa un gran peligro ambiental menor que la de NiCd debido a la ausencia de cadmio tóxico. La extracci'on y su procesamiento puede plantear otros tipos de impacto ambiental, sin embargo, la mayoría de níquel industrial es reciclado, debido a la relativamente fácil recuperación mediante electroimanes. Sobre el n'iquel, se ha de tener en cuenta que tiene efectos sobre la piel y que puede producir c'ancer de pulm'on si se inhala en altas cantidades.\\

\section{Elementos introducidos}
Sobre los componentes introducidos, la gran mayor'ia cumplen la directiva RoHS actual. El 'unico componente que puede introducir un poco de sospecha son las bater'ias. Estas bater'ias est'an compuestas por litio, un elemento neurot'oxico y da'nino para el ri'non. Aunque el litio puede conllevar riesgo, su impacto ambiental es m'inimo. \\

En Estados Unidos se ha declarado que se pueden desechar de forma segura como residuo normal, por el contrario, los gobiernos europeos advierten del potencial para contaminar aguas subterr'aneas debido a que contienen metales base como el cobalto. Para extender el uso de las bater'ias, contamos con los m'odulos PCM. Estos m'odulos nos ayudar'an a alargar la vida de nuestras celdas de energ'ia.

\newpage
\chapter*{Conclusiones}
\addcontentsline{toc}{chapter}{Conclusiones}
A lo largo de este proyecto se han cumplido todos los objetivos propuestos. Obteniendo un producto apto para su vuelta al sector educacional. Aunque restaurar una plataforma y volverla a poner al día ha sido una labor que conllevado varios quebraderos de cabeza, este proyecto demuestra que es realizable. Durante la ejecuci'on de este proyecto se ha observado que las plataformas rob'oticas se quedan obsoletas a causa de sus unidades de procesamiento desfasadas, mientras que los componentes m'as b'asicos como los controladores de motores o los sensores siguen estando operativos. Todo esto hace ver que quiz'a hace falta un cambio en el razonamiento de la rob'otica, de tal manera que las unidades de procesamiento puedan ser actualizadas sin afectar al resto de los elementos de un robot.\\

A medida que se ha ido realizando el proyecto, trabajando con el robot, pensando mejoras y aplic'andolas, uno se da cuenta de las deficiencias del producto original y comprende la evoluci'on que ha seguido la plataforma WifiBot hasta el presente. El cambio m'as notorio es el hecho que los nuevas plataformas creadas por la empresa francesa est'an constituidas por un 'unico bloque (sin la necesidad de crear dos hemisferios). Durante el dise'no del RosPiBot, en el momento de la ubicar sus componentes, se destino gran parte del tiempo a encontrar la manera que el switch, la Raspberry Pi y el router pudiesen convivir en el mismo espacio. Adem'as, disponer de una barra met'alica que atraviesa transversalmente el robot, dificult'o enormemente su desarrollo.\\

Por 'ultimo, el hecho de ''Rosificar'' cualquier plataforma rob'otica es una apuesta segura hacia su desarrollo. La gran comunidad que hay detr'as de ROS y el nivel de conocimientos del que dispone, son motivos suficientes utilizar esta herramienta. Incluso con plataformas desactualizadas, añadir ROS puede originar un resurgimiento. \\

Como futuros proyectos a realizar, se sugieren: Odometria de robots con conducción per deslizamiento, Teleoperación a distancia, Navegación autónoma o incluso añadir nuevos accesorios como sensores de calidad del aire o brazos robóticos. 
...
Un proyecto como este no se podría haber realizado sin el apoyo de familiares ni amigos. Ya que son ellos los que me han prestado ayuda y me han dado 'animos durante toda la realizaci'on de este trabajo.\\

Agradecer la oportunidad que los profesores  \textbf{Cecilio Angulo} y \textbf{Manuel Velasco}  me han ofrecido al permitirme poder realizar todas las modificaciones deseadas en el  WifiBot. Tambi'en por el apoyo a la hora de resolver dudas o aportar ideas al proyecto. Adem'as agradecer a los miembros de la asociaci'on \textbf{AESS Estudiants} todo el tiempo que me han dedicado para que este proyecto fuese posible. Compartiendo su experiencia para solucionar los problemas que aparec'ian dia a dia.
...
\bibitem{guia} \textsc{Nexter Robotics}, 
\textit{Wifibot4G Quick Start Guide}
Francia, 2006
\url{http://rbouraoui.free.fr/wifibot/wifibot4Genglishguide.pdf}

\bibitem{motor} \textsc{Lynxmotion}, 
\textit{Data sheet for: GHM-04 7.2vdc 50:1 175rpm 6mm shaft}
Estados unidos, 2006
\url{http://vallejo.cc/proyectos/cellbot/motor_lynxmotion_7_2V.pdf}

\bibitem{encoder} \textsc{US Digital}, 
\textit{E4P OEM Miniature Optical Kit Encoder}
Estados Unidos, 2005
\url{http://www.usdigital.com/assets/datasheets/E4P_datasheet.pdf?k=635367475934568087}

\bibitem{sharp} \textsc{Sharp Corporation}, 
\textit{Sharp Corporation Sharp Corporation}
Jap'on, 2001
\url{http://www.erasme.org/IMG/gp2y0a02_e.pdf}

\bibitem{sonar} \textsc{Kingsin Technologies Co.}, 
\textit{KS101B/KS103/KS103S Ultra Sonic Range Finder Technical Specification}
China, 2011
\url{http://www.dauxi.com/KS10X-V110_EN.pdf}

\bibitem{router} \textsc{TP-LINK}, 
\textit{TL-WR702N 150Mbps Wireless N Nano Router User Guide}
China, 2014
\url{http://www.tp-link.com/resources/document/TL-WR702N_V1_User_Guide_1910010880.pdf}

\bibitem{counter} \textsc{LSI Computer Systems, Inc.}, 
\textit{32-Bit Quadrature Counter With Serial Interface}
Estados Unidos, 2012
\url{http://www.lsicsi.com/pdfs/Data_Sheets/LS7366R.pdf}

\bibitem{bidirectional} \textsc{Philips Semiconductors N.V.}, 
\textit{Bi-directional level shifter for I$^{2}$C-bus and other
systems. Application Note AN97055}
Países Bajos, 1997
\url{http://www.adafruit.com/datasheets/an97055.pdf}

\bibitem{bss138} \textsc{Fairchild Semiconductors}, 
\textit{BSS138 N-Channel Logic Level Enhancement Mode Field Effect Transistor}
Estados Unidos, 2005
\url{http://www.adafruit.com/datasheets/BSS138.pdf}

\bibitem{ADC} \textsc{Microchip Technology Inc.}, 
\textit{16-Bit, Multi-Channel $\Delta \Sigma$ Analog-to-Digital Converter with I$^{2}$C Interface and On-Board Reference}
Estados Unidos, 2009
\url{http://ww1.microchip.com/downloads/en/DeviceDoc/22226a.pdf}

\bibitem{display} \textsc{Texas Instruments Inc.}, 
\textit{LM3914 Dot/Bar Display Driver}
Estados Unidos, 2000
\url{http://www.ti.com/lit/ds/symlink/lm3914.pdf}
...
\bibitem{regulador} \textsc{Texas Instruments Inc.}, 
\textit{LM2596 SIMPLE SWITCHER Power Converter 150 kHz
3A Step-Down Voltage Regulator}
Estados Unidos, 1999
\url{http://www.ti.com/lit/ds/symlink/lm2596.pdf}

\bibitem{driver} \textsc{STMicroelectronics N.V.}, 
...
\textbf{Filezilla} Linux, Mac, Windows\\
...
\textbf{KiCad} Linux y Windows\\
Entorno de software usado para el diseño de circuitos eléctricos y de c'odigo abierto.\\

\textbf{Photoshop CS2} Windows\\
Editor de gráficos rasterizados desarrollado para el retoque de fotografías y gráficos.\\

\textbf{Putty} Linux, Mac y Windows\\
Cliente SSH, Telnet, rlogin, y TCP raw con licencia libre. Permite conexi'on remota con otro ordenador.\\

\textbf{Solidworks 2010} Windows\\
